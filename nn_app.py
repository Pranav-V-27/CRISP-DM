...
model_filename = 'sharks_model.h5'
...
label_encoder, columns = joblib.load('sharks_preprocessing.joblib')
...
# Title
st.title("Shark Tank India - Predicting Individual Shark's Investment Probability using Neural Network")
...
new_data = pd.get_dummies(new_data, columns=['Pitchers Average Age'])  # Include other columns as needed

# Ensure the columns match the encoded columns from the preprocessing steps
missing_columns = set(columns) - set(new_data.columns)
for column in missing_columns:
    new_data[column] = 0  # Add missing columns with all zeros

if st.button("Make Prediction"):
    prediction = loaded_model.predict(new_data)
    confidence = loaded_model.predict_proba(new_data)[0][1]
    st.subheader("Prediction")
    if prediction[0] == 1:
        st.write(f"Accepted Offer✔ with confidence score {int(confidence*10000)/100}%")
        
        # Show the bar chart of individual shark investment probability
        output = loaded_model.predict(X[0:1])
...
markdown_string = """## Shark Tank Offer Outcome Prediction Model (Neural Network)

This is a Streamlit app for predicting the outcome of a pitch on Shark Tank India using a trained neural network model.
